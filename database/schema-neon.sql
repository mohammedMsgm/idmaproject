-- Schema for IDMA Platform - Neon PostgreSQL Compatible
-- Note: Database creation is handled by Neon, we only create tables and data
...
-- Insert sample doctors (check if they don't exist first)
...
) AS temp_users(name, email, password, user_type, avatar)
WHERE NOT EXISTS (SELECT 1 FROM users WHERE email = temp_users.email);

-- Insert doctor specializations
INSERT INTO doctor_profiles (user_id, specialization, experience_years)
SELECT u.id, p.specialization, p.experience_years
FROM (VALUES
...
) AS p(email, specialization, experience_years)
...
SELECT temp_patients.name, temp_patients.email, temp_patients.password, temp_patients.user_type, temp_patients.avatar, d.id
FROM (VALUES
...
) AS temp_patients(name, email, password, user_type, avatar, doctor_email)
JOIN users d ON d.email = temp_patients.doctor_email AND d.user_type = 'doctor'
WHERE NOT EXISTS (SELECT 1 FROM users WHERE email = temp_patients.email);
